...
| Controller reconnect and mapping | Deterministic slot/reconciliation, missed-removal, held-input-release, mapping/persistence, foreground-resume, simulator GameController assignment, and simulator foreground-retention tests passed; the revised source default maps left shoulder to Z and fixed right shoulder to 50% analog R without a digital click; iPhoneOS and Simulator builds passed before the revised shoulder mapping | Hands-on Bluetooth, wired, and natural-sleep acceptance remains: revised shoulder mapping and run-and-spray, player 1 retention/reclaim, two-controller preservation, pressure triggers, disconnect release, active reconnect, foreground reconnect, touch hiding, mapping, and Modern C-stick behavior |
